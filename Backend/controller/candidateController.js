...
export const createCandidate=async(req,res)=>{
    ...
    const {candidateName, candidateEmail, candidateAddress, candidateDOB,candidateMobileNo,candidateDescription,categoryId,partyId}=req.body
    let fileName;
    if(req.file){
    ...
    try{
       const candidateData= await Candidate.create({candidateName, candidateEmail, candidateAddress, candidateDOB,candidateMobileNo,candidateDescription,imageUrl:fileName, userId:userId,categoryId:categoryId,partyId:partyId })
...
// Fetch candidates by party
export const fetchCandidatesByParty = async (req, res) => {
  const { partyId } = req.params; // Get the partyId from request parameters
  try {
    const response = await Candidate.findAll({
      where: { partyId }, // Filter candidates by partyId
      include: [
        { model: User, attributes: ['id', 'email', 'username'] },
        { model: Category, attributes: ['categoryName'] },
        { model: Party, attributes: ['partyName'] },
    ...
    res.status(200).json({
      message: "Successfully fetched candidates for the party",
      data: response
    });
  } catch (error) {
    res.status(500).json({
